...
// Tally 改动：两条配额按池分（Gemini 池 / Claude 池各取剩得少的窗口，原版把 gemini 周窗口填进了标「Claude 池」的那条）；缺 quotaInfo 的模型不算读数；摘要里没有认得的 bucket 就接着找，且先在所有端点上问摘要再退到模型配置；云端两个地址都被 401 / 403 拒时报「登录已过期」（原版的空 catch 把它吞了，只剩一句笼统的取不到）。
import Foundation
import os
...
struct AntigravityUsageProvider: UsageProvider {
    ...
    private func fetchQuotaFromLanguageServer(now: Date) async throws -> (UsageSnapshot, LSEndpoint)? {
        return try await withTimeout(seconds: 8) {
            ...
            var endpoints: [LSEndpoint] = []
            for discovery in discoveries {
                for port in discovery.ports {
                    endpoints.append(LSEndpoint(scheme: "https", port: port, csrf: discovery.csrf))
                    endpoints.append(LSEndpoint(scheme: "http", port: port, csrf: discovery.csrf))
                }
                if let extPort = discovery.extensionPort {
                    endpoints.append(LSEndpoint(scheme: "http", port: extPort, csrf: discovery.csrf))
                }
            }

            // 先在所有端点上问配额摘要，都没有再退到模型配置：IDE 会起两个语言服务器，答得了摘要的常是第二个，
            // 逐个端点退的话，第一个的模型配置先答上来，第二个的摘要就永远问不到了
            for ep in endpoints {
                if let summary = try await callLS(scheme: ep.scheme, port: ep.port, csrf: ep.csrf, method: "RetrieveUserQuotaSummary"),
                   let snapshot = Self.parseQuotaSummary(summary, now: now) {
                    return (snapshot, ep)
                }
            }

            for ep in endpoints {
                if let status = try await callLS(scheme: ep.scheme, port: ep.port, csrf: ep.csrf, method: "GetUserStatus") {
                    if let snapshot = parseUserStatus(status, now: now) {
                        return (snapshot, ep)
                    }
                }

                if let fallback = try await callLS(scheme: ep.scheme, port: ep.port, csrf: ep.csrf, method: "GetCommandModelConfigs") {
    ...
    /// 测试直接喂回包，所以不是 private。
    static func parseQuotaSummary(_ data: Data, now: Date) -> UsageSnapshot? {
        ...
        // 一个认得的 bucket 都没有：这不是读数，交回 nil 让调用方接着问下一个端点
        ...
        // 两条各是一个池（界面标「Gemini 池」「Claude 池」），每个池有 5 小时与周两个窗口，显示剩得少的那个，重置时刻跟着它。
        // 原版两条都填 gemini（5 小时、周），第二条却标成 Claude 池——Claude 池用光了也看不见
        func tightest(_ ids: [String]) -> UsageLimit? {
            guard let entry = ids.compactMap({ pooled[$0] }).min(by: { $0.fraction < $1.fraction }) else { return nil }
            return UsageLimit(used: (1 - max(0, min(1, entry.fraction))) * 100, limit: 100, resetsAt: entry.resetTime)
        }
        snapshot.sessionLimit = tightest(["gemini-5h", "gemini-weekly"])
        snapshot.weekLimit = tightest(["3p-5h", "3p-weekly"])
        ...
        if let cascade = userStatus["cascadeModelConfigData"] as? [String: Any],
           let clientConfigs = cascade["clientModelConfigs"] as? [[String: Any]] {
            for config in clientConfigs {
                if let modelConfig = Self.parseModelConfig(config) {
                    configs.append(modelConfig)
                }
            }
        }

        return Self.buildSnapshot(from: configs, now: now)
    }
    ...
    /// 测试直接喂字典，所以不是 private。
    static func parseModelConfig(_ obj: [String: Any]) -> AntigravityModelConfig? {
        guard let label = (obj["label"] as? String)?.trimmingCharacters(in: .whitespaces),
              !label.isEmpty else { return nil }

        // 没有 quotaInfo 的模型不是「用光了」，是这里没有它的配额读数；有 quotaInfo 而缺 remainingFraction 才按 0（proto3 的 JSON 不写零值）
        guard let quota = obj["quotaInfo"] as? [String: Any] else { return nil }
        ...
        return try await withThrowingTaskGroup(of: UsageSnapshot.self) { group in
            group.addTask {
                ...
                for baseURL in baseURLs {
                    if let data = await self.cloudCodeData(path: "/v1internal:retrieveUserQuotaSummary", baseURL: baseURL, token: token, authRejected: &authRejected) {
                ...
                for baseURL in baseURLs {
                    if let data = await self.cloudCodeData(path: "/v1internal:fetchAvailableModels", baseURL: baseURL, token: token, authRejected: &authRejected) {
                ...
                throw UsageError.notConfigured(authRejected ? "Antigravity auth expired" : "Unable to fetch Antigravity usage")
            }
    ...
    private func cloudCodeCall(path: String, baseURL: String, token: AntigravityKeychainToken) async throws -> Data? {
        ...
        do {
            ...
            // 上面 401 / 403 抛的「登录已过期」要带出去；原来的空 catch 连它一起吞了，界面只剩一句笼统的取不到
            throw error
        } catch {
        }
        return nil
    ...
    /// 同 `cloudCodeCall`，但 401 / 403 不马上放弃：两个地址有一个认就行，记一笔接着试，都不行时由调用方决定怎么说。
    private func cloudCodeData(path: String, baseURL: String, token: AntigravityKeychainToken, authRejected: inout Bool) async -> Data? {
        do {
            return try await cloudCodeCall(path: path, baseURL: baseURL, token: token)
        } catch {
            authRejected = true
            return nil
        }
    ...
    private func parseCloudCodeModels(_ data: Data, now: Date) -> UsageSnapshot? {
        ...
        for (key, value) in models {
            ...
            // 同 parseModelConfig：没有 quotaInfo 就没有读数，不当用光
    ...
    /// 测试直接喂配置，所以不是 private。一条配置都没有时不是读数，返回 nil 让调用方接着找。
    static func buildSnapshot(from configs: [AntigravityModelConfig], now: Date) -> UsageSnapshot? {
        guard !configs.isEmpty else { return nil }
        var snapshot = UsageSnapshot()
        snapshot.lastUpdated = now

        // Pool into Gemini (Session) and non-Gemini (Claude)。nil 是池里一条模型都没有；有模型而没用过（剩 100%）也是读数
        ...
        for config in configs {
            ...
            if pool == "gemini" {
                if fraction < geminiFraction ?? .infinity {
                    geminiFraction = fraction
                    geminiReset = config.resetTime
                }
            } else {
                if fraction < claudeFraction ?? .infinity {
        ...
        // 两条各是一个池，和界面标签「Gemini 池」「Claude 池」对上；原版只有 Gemini 没用过时才把 Claude 池塞进第一条，标签就错了
        if let geminiFraction {
            snapshot.sessionLimit = UsageLimit(used: (1 - geminiFraction) * 100, limit: 100, resetsAt: geminiReset)
        }
        if let claudeFraction {
            snapshot.weekLimit = UsageLimit(used: (1 - claudeFraction) * 100, limit: 100, resetsAt: claudeReset)
        }
