...
- **文件夹要用户点过**：打开开关时弹选择面板，默认指到系统截图位置（`com.apple.screencapture` 的 `location`，没设过是桌面），点「用这个文件夹」才开。桌面、文稿、下载受隐私保护，用户在选择面板里点选这一下，系统把授权记在那个目录上（`com.apple.macl`），跨重启都能读、不弹框、不用安全书签。取消就不开。读不了（授权没了、目录没了）时原因亮在设置里，不默默失效。
- **认标记不认名字**：新出现的文件带扩展属性 `com.apple.metadata:kMDItemIsScreenCapture` = true 才算（系统截图和 CleanShot 这类都写，复制后还在）；直接 `getxattr` 读，不等 Spotlight 建索引。文件名随系统语言和用户设置变，不看。
- **只收新的**：开始盯的那一刻把已有文件的 inode 记成见过；之后在访达里改名还是同一个 inode，不重复放。截图的缩略图还浮在角上时文件在临时目录，消失后才挪进文件夹，所以一挪进来就是完整文件，不用等大小稳定。
- 目录用 `DispatchSource`（`O_EVTONLY` + `.write`）盯，一次截图会连着触发几次，攒 0.5 秒再扫一遍比对。盯的是文件描述符不是路径：目录被删、被挪走（iCloud 桌面同步、外接盘拔掉）之后再也不来事件，所以 `.delete` / `.rename` 也收，收到就停下、设置里说重新选；扫的时候读不了（授权被收回）同样亮出来。开关、文件夹、文件架开关变了才重起（`start` 同一文件夹幂等，不然每次改设置都会把这段时间到的截图记成见过）。
...
```bash
swift test --filter 'ShelfStoreTests|ScreenshotWatcherTests|PreferencesTests'
...
用例：加两份同名文件各自成项且副本存在、重开实例索引还在、删除连文件一起删、清空；没开不收、可见时缩略图在内存、不可见放掉、关掉索引清空但磁盘保留；过期判定；`cutURL` 来源还在时交来源、来源没了退回架上那份；老索引缺 `source` 能解出来；「移动到…」搬的是来源、架上那份跟着清掉、废纸篓里不多东西，目标已有同名文件时报错且什么都不动；截图（带标记才算、名字像截图不算、隐藏文件不列、开始盯之前就在的不算、改名不重复放、读不了的文件夹把原因亮出来、盯着的目录被删掉后停下并说原因）。截图：空态虚线框；手动：从访达拖一个文件到刘海上 → 面板展开到文件架并出现该项；把它拖回桌面得到一份副本，来源还在；**按住 ⌘ 拖回桌面 → 桌面有、架上那件消失、来源原处没有了且废纸篓里也没有**（它是被改名过去的）；右键 AirDrop 出系统面板；`open -a Tally /tmp/x.txt` → 闭合态弹「放进文件架 x.txt」、文件架里多一项；退出 Tally 后再跑一次同样的命令 → Tally 被拉起来且这一项照样进来。
