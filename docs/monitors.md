...
- **素材**：Stride 定稿的逐帧图原样打包（冲刺的速度线加粗到 1.6pt、提亮成 `#ffcf70` / `#b35200`，睡觉的 Z 加粗到 1.45pt、睡帽和 Z 提亮成 `#9be3ff` / `#0a5fc0` 后重新导出），`Resources/stride/{dark,light}/<状态>-NN.png`（68 × 36px，按 34 × 18pt 显示；睡觉、跑步、冲刺各 8 帧，生气 12 帧；深浅两套共 72 张、288 KB，`build-app.sh` 整个目录拷进 app）。彩色帧不能用模板图，按状态栏按钮的 `effectiveAppearance` 选深浅那一套，外观变了（KVO）换套。帧图拼上蛋的成品按帧缓存，蛋的样子（深浅、内存整数百分比、颜色档）变了才整份作废，换帧只换缓存里的图、不每帧现画；拼不出来（位图建不出来）时缓存不带蛋的帧并记日志，不每帧重试。缺帧图时日志列出缺的是哪几张。
